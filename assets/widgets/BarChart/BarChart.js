...
Dashing.BarChart = (function(superClass) {
  ...
  BarChart.prototype.ready = function() {
    ...
    return this.myChart = new Chart(this.ctx).Bar(this.myData, $.extend({
      responsive: false,
      barShowStroke: true,
  ...
  BarChart.prototype.onData = function(data) {
    ...
    if (this.myChart && data.datasets) {
      for (i = k = 0, ref = this.myChart.datasets.length - 1; 0 <= ref ? k <= ref : k >= ref; i = 0 <= ref ? ++k : --k) {
        for (j = l = 0, ref1 = this.myChart.datasets[i].bars.length - 1; 0 <= ref1 ? l <= ref1 : l >= ref1; j = 0 <= ref1 ? ++l : --l) {
          this.myChart.datasets[i].bars[j].value = data.datasets[i].data[j];
        }
      }
